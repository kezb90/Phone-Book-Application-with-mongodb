...
if "contact" in collections:
    contact_collection = mongodb_database.get_collection("contact")
else:
    contact_collection = mongodb_database["contact"]
...
class PhoneBookApp(QWidget):
    ...
    def on_item_changed(self, item):
        ...
        if reply == QMessageBox.Yes:
            ...
            find_condition = {"_id": ObjectId(id)}
            first_name = self.results_table.item(row, 1).text()
            last_name = self.results_table.item(row, 2).text()
            phone_number = self.results_table.item(row, 3).text()
            address = self.results_table.item(row, 4).text()
            update_operation = {
                "$set": {
            ...
            result = contact_collection.update_one(find_condition, update_operation)
            ...
            QMessageBox.information(
                self,
                "Contact Updated",
                f"For Contact ID: {id}, {header_item} changed to: {new_value}",
            )
        else:
            self.search_contacts()
        ...
        if item is not None:
            ...
            if col == 5:
                ...
                if reply == QMessageBox.Yes:
                    ...
                    result = contact_collection.delete_one(delete_condition)
                    self.search_contacts()
    ...
    def add_contact(self):
        ...
        # Insert the new_contact data into the 'contact' collection
        result = contact_collection.insert_one(new_contact)
        print(f"Contact inserted with ID: {result.inserted_id}")
    ...
    def search_contacts(self):
        ...
        find_condition = {
            "first_name": {
                "$regex": first_name_search_term,
                "$options": "i",
            },  # Case-insensitive regex
            "last_name": {
                "$regex": last_name_search_term,
                "$options": "i",
            },  # Case-insensitive regex
        }

        # Find documents that match both conditions
        contacts = contact_collection.find(find_condition)
        ...
        for row, contact in enumerate(contacts):
            ...
            item = QTableWidgetItem(str(contact["_id"]))
            ...
            self.results_table.setItem(
                row, 3, QTableWidgetItem(contact["phone_number"])
            )
